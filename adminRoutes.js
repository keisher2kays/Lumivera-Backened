...
// // backend-lumivera/adminRoutes.js
...
//     const newUser = await User.create({ name, username, password, role: formattedRole });

//     return res.status(201).json({
//       success: true,
//       message: 'Team member created successfully',
//       user: { _id: newUser._id, name: newUser.name, username: newUser.username, role: newUser.role },
//     });
//   } catch (error) {
...
//         user: { name: dbUser.name, username: dbUser.username, role: dbUser.role },
//       });
//     }
...
// // NEW — delete a lead once it's been handled/closed
...
//     if (!name || !category || priceZimUSD === undefined || priceZimUSD === '') {
...
//       name,
//       category,
//       priceZimUSD: Number(priceZimUSD),
//       priceUKGBP: priceUKGBP ? Number(priceUKGBP) : undefined,
//       stockQuantity: qty,
//       inStock: qty > 0,
//       description,
//       specs: specs ? specs.split(',').map((s) => s.trim()).filter(Boolean) : [],
//     });

//     res.status(201).json({ success: true, message: 'Product added successfully', product });
//   } catch (err) {
//     console.error('Error creating product:', err.message);
//     res.status(500).json({ success: false, message: err.message || 'Server error creating product' });
//   }
// });
...
// // NEW — manually adjust stock quantity from the portal (restock, correction, etc.)
...
// // NEW — remove a single client's booking from an installation day
...
//     if (installation.clients.length === before) {
//       return res.status(404).json({ success: false, message: 'Client booking not found' });
//     }
...
//   } catch (err) {
//     console.error('Error removing booking:', err.message);
//     res.status(500).json({ success: false, message: 'Server error removing booking' });
//   }
// });
...
//     if (!scheduledDate) {
//       return res.status(400).json({ success: false, message: 'A scheduled date is required.' });
//     }

//     const packageName = items && items.length > 0
//       ? items.map((i) => `${i.name} (x${i.quantity})`).join(', ')
//       : 'Cart Checkout Package';
...
//     if (installation.bookedSlots >= installation.maxSlots || !installation.isAvailable) {
//       return res.status(400).json({ success: false, message: 'Selected date is fully booked.' });
//     }

//     installation.clients.push({
//       clientName: customerName,
//       phone: customerPhone,
//       packageName: `${packageName} | Location: ${location || 'N/A'} | Notes: ${notes || 'None'} | Total: ${totalAmount || ''}`,
//     });

//     installation.bookedSlots += 1;
//     if (installation.bookedSlots >= installation.maxSlots) {
//       installation.isAvailable = false;
//     }
...
//   } catch (error) {
//     return res.status(500).json({ success: false, message: 'Failed to retrieve installations' });
//   }
// });
